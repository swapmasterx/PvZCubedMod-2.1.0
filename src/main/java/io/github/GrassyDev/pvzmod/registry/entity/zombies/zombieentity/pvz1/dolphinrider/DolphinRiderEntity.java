...
import static io.github.GrassyDev.pvzmod.sound.PvZSounds.DOLPHINJUMPEVENT;
import static io.github.GrassyDev.pvzmod.sound.PvZSounds.DOLPHINWATEREVENT;

public class DolphinRiderEntity extends PvZombieEntity implements GeoEntity {
	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
		if (this.isInsideWaterOrBubbleColumn() || this.removeDolphin) {
			if (this.getDolphinStage()) {
				event.getController().setAnimation(RawAnimation.begin().thenLoop("dolphinrider.ducky"));
			}
			else {
				event.getController().setAnimation(RawAnimation.begin().thenLoop("dolphinrider.ducky2"));
			}
			if (this.isIced) {
			...
			if (this.getDolphinStage()) {
				if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
					event.getController().setAnimation(RawAnimation.begin().thenLoop("dolphinrider.walking"));
				} else {
					event.getController().setAnimation(RawAnimation.begin().thenLoop("dolphinrider.idle"));
				}
			} else {
				if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
					event.getController().setAnimation(RawAnimation.begin().thenLoop("dolphinrider.walking2"));
				} else {
					event.getController().setAnimation(RawAnimation.begin().thenLoop("dolphinrider.idle2"));
				}
			}
	...
	public void tick() {
		...
		if (this.getAttacking() == null && !(this.getHypno()) && this.isAlive()){
			if (this.CollidesWithPlant(0.1f, 0f) instanceof GardenChallengeEntity){
				...
				else if (this.CollidesWithPlant(0.1f, 0f) != null && !this.hasStatusEffect(StatusHolder.BOUNCED_HOLDER) && !(this.CollidesWithPlant(0.1f, 0f) instanceof TangleKelpEntity) && this.getDolphinStage() && this.isInsideWaterOrBubbleColumn() && !removeDolphin){
			...
			else if (!this.isInsideWaterOrBubbleColumn() && this.getDolphinStage() && !this.hasStatusEffect(StatusHolder.BOUNCED_HOLDER) && !removeDolphin && this.touchedWater){
				Vec3d vec3d = new Vec3d(0.5, 0.8, 0.0).rotateY(-this.getYaw() * (float) (Math.PI / 180.0) - ((float) (Math.PI / 2)));
				this.addVelocity(vec3d.getX(), vec3d.getY(), vec3d.getZ());
				this.removeDolphin = true;
				this.waitDolphinTick = 5;
				...
				else if (this.CollidesWithPlant(0.1f, 0f) != null && !this.hasStatusEffect(StatusHolder.BOUNCED_HOLDER) && (PLANT_LOCATION.get(this.CollidesWithPlant(0.1f, 0f).getType()).orElse("normal").equals("tall") || PLANT_LOCATION.get(this.CollidesWithPlant(0.1f, 0f).getType()).orElse("normal").equals("flying")) && !this.isOnGround() && !this.isInsideWaterOrBubbleColumn()){
				...
				else if (this.CollidesWithPlant(0.1f, 0f) != null && !this.hasStatusEffect(StatusHolder.BOUNCED_HOLDER) && ((this.isInsideWaterOrBubbleColumn() && !this.getDolphinStage()) || this.isOnGround())){
		...
		if (--waitDolphinTick <= 0 && removeDolphin && (this.isInsideWaterOrBubbleColumn() || this.isOnGround())){
	...
	public boolean damage(DamageSource source, float amount) {
		if (!super.damage(source, amount)) {
			...
			if (this.getRecentDamageSource().isType(PvZDamageTypes.HYPNO_DAMAGE) && !(this.getHypno())) {
				...
				DolphinRiderEntity hypnotizedZombie = (DolphinRiderEntity) hypnoType.create(getWorld());
