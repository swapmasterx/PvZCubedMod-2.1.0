...
public class ShootingMelonSliceEntity extends PvZProjectileEntity implements GeoEntity {
	...
	public ShootingMelonSliceEntity(EntityType<? extends ThrownItemEntity> entityType, World world) {
		super(entityType, world);
		this.setNoGravity(false);
	}

	public ShootingMelonSliceEntity(World world, LivingEntity owner) {
		super(EntityType.SNOWBALL, owner, world);
	}

	@Environment(EnvType.CLIENT)
	public ShootingMelonSliceEntity(World world, double x, double y, double z, float yaw, float pitch, int interpolation, boolean interpolate, int id, UUID uuid) {
...
//				}
	...
	public void hitEntities() {
		...
		while (true) {
			...
			if (!getWorld().isClient && entity instanceof Monster monster &&
				...
				float damage = PVZCONFIG.nestedProjDMG.melonsliceDMG();
	...
	private void raycastExplode() {
		...
		List<LivingEntity> list = this.getWorld().getNonSpectatingEntities(LivingEntity.class, this.getBounds().expand(5));
		...
		while (true) {
			...
			} while (this.squaredDistanceTo(livingEntity) > 5);
			float damage = PVZCONFIG.nestedProjDMG.melonsliceSDMG() * damageMultiplier;
