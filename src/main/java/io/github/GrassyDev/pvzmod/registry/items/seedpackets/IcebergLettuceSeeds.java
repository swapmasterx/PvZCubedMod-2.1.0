...
public class IcebergLettuceSeeds extends SeedItem implements FabricItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				...
				List<PlantEntity> list = getWorld().getNonSpectatingEntities(PlantEntity.class, PvZEntity.ICEBERGLETTUCE.getDimensions().getBoxAt(plantEntity.getPos()));
				if (list.isEmpty()) {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			...
			list = getWorld().getNonSpectatingEntities(PlantEntity.class, PvZEntity.ICEBERGLETTUCE.getDimensions().getBoxAt(plantEntity.getPos()));
		}
