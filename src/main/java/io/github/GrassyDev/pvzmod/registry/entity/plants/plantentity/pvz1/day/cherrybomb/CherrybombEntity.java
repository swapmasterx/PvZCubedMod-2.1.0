...
public class CherrybombEntity extends PlantEntity implements GeoAnimatable {
	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
        ...
        if (i > 0) {
            event.getController().setAnimation(new RawAnimation().playOnce("cherrybomb.explode"));
        } else {
            event.getController().setAnimation(new RawAnimation().loop("cherrybomb.idle"));
        }
	...
	protected void applyDamage(DamageSource source, float amount) {
		if (this.getTarget() == null  || source.isOutOfWorld()) {
