...
public class JetpackEntity extends PvZombieEntity implements GeoAnimatable {
	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
		if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
			event.getController().setAnimation(new RawAnimation().loop("jetpack.flying"));
		} else {
			event.getController().setAnimation(new RawAnimation().loop("jetpack.idle"));
		}
