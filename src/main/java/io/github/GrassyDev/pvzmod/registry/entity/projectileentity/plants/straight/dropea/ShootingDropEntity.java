...
public class ShootingDropEntity extends PvZProjectileEntity implements GeoEntity {
...
@Override
    ...
    public ShootingDropEntity(World world, double x, double y, double z, float yaw, float pitch, int interpolation, int id, UUID uuid) {
    ...
    public void tick() {
		...
		if (random <= 0.25) {
			for (int j = 0; j < 1; ++j) {
				...
				this.getWorld().addParticle(ParticleTypes.FALLING_DRIPSTONE_WATER, this.getX(), this.getY(), this.getZ(), d, e, f);
			}
		}
