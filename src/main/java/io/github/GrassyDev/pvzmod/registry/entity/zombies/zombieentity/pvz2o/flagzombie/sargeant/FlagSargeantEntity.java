...
public class FlagSargeantEntity extends SummonerEntity implements GeoEntity {
...
//	}
	...
	protected abstract class CastSpellGoal extends Goal {
		...
		public boolean canStart() {
			...
			if (livingEntity != null && livingEntity.isAlive() && !FlagSargeantEntity.this.hasStatusEffect(StatusHolder.FROZEN_HOLDER) && !FlagSargeantEntity.this.hasStatusEffect(StatusHolder.STUN_HOLDER)) {
		...
		public boolean shouldContinue() {
			LivingEntity livingEntity = FlagSargeantEntity.this.getTarget();
			return livingEntity != null && livingEntity.isAlive() && this.spellCooldown > 0 && !FlagSargeantEntity.this.hasStatusEffect(StatusHolder.FROZEN_HOLDER) && !FlagSargeantEntity.this.hasStatusEffect(StatusHolder.STUN_HOLDER) && FlagSargeantEntity.this.getTypeCount() < 4;
		}
    ...
    class summonZombieGoal extends CastSpellGoal {
        ...
        protected void castSpell() {
            ...
            for(int b = 0; b < 1; ++b) { // 1 Screendoor
                ...
                BlockPos blockPos = FlagSargeantEntity.this.getBlockPos().add((int) vec3d.getX(), 0, (int) vec3d.getZ());
				...
				BlockPos blockPos2 = FlagSargeantEntity.this.getBlockPos().add((int) vec3d2.getX(), 1, (int) vec3d2.getZ());
				if (!getWorld().getBlockState(blockPos).isOf(Blocks.AIR) && !getWorld().getBlockState(blockPos).isOf(Blocks.CAVE_AIR) && !getWorld().getBlockState(blockPos2).isOf(Blocks.AIR) && !getWorld().getBlockState(blockPos2).isOf(Blocks.CAVE_AIR)){
					...
					blockPos = FlagSargeantEntity.this.getBlockPos().add((int) vec3d.getX(), 0, (int) vec3d.getZ());
				}
