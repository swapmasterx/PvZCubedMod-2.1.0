...
public class FlagPeasantEntity extends SummonerEntity implements GeoEntity {
	...
	public void tick() {
		if (this.hasStatusEffect(StatusHolder.POISON_HOLDER) || this.hasStatusEffect(StatusEffects.POISON)){
...
//	}
	...
	public boolean damage(DamageSource source, float amount) {
		if (!super.damage(source, amount)) {
			...
			if (this.getRecentDamageSource().isType(PvZDamageTypes.HYPNO_DAMAGE) && !(this.getHypno())) {
				...
				FlagPeasantEntity hypnotizedZombie = (FlagPeasantEntity) hypnoType.create(getWorld());
	...
	protected abstract class CastSpellGoal extends Goal {
		...
		public boolean canStart() {
			...
			if (livingEntity != null && livingEntity.isAlive() && !FlagPeasantEntity.this.hasStatusEffect(StatusHolder.FROZEN_HOLDER) && !FlagPeasantEntity.this.hasStatusEffect(StatusHolder.STUN_HOLDER)) {
		...
		public boolean shouldContinue() {
			LivingEntity livingEntity = FlagPeasantEntity.this.getTarget();
			return livingEntity != null && livingEntity.isAlive() && this.spellCooldown > 0 && !FlagPeasantEntity.this.hasStatusEffect(StatusHolder.FROZEN_HOLDER) && !FlagPeasantEntity.this.hasStatusEffect(StatusHolder.STUN_HOLDER) && FlagPeasantEntity.this.getTypeCount() < 4;
		}
    ...
    class summonZombieGoal extends CastSpellGoal {
        ...
        protected void castSpell() {
            ...
            ServerWorld serverWorld = (ServerWorld) FlagPeasantEntity.this.getWorld();
            for(int b = 0; b < 1; ++b) { // 1 Screendoor
                ...
                BlockPos blockPos = FlagPeasantEntity.this.getBlockPos().add((int) vec3d.getX(), 0, (int) vec3d.getZ());
				...
				BlockPos blockPos2 = FlagPeasantEntity.this.getBlockPos().add((int) vec3d2.getX(), 1, (int) vec3d2.getZ());
				if (!getWorld().getBlockState(blockPos).isOf(Blocks.AIR) && !getWorld().getBlockState(blockPos).isOf(Blocks.CAVE_AIR) && !getWorld().getBlockState(blockPos2).isOf(Blocks.AIR) && !getWorld().getBlockState(blockPos2).isOf(Blocks.CAVE_AIR)){
					...
					blockPos = FlagPeasantEntity.this.getBlockPos().add((int) vec3d.getX(), 0, (int) vec3d.getZ());
				}
				PeasantEntity screendoorEntity = (PeasantEntity) screen.create(FlagPeasantEntity.this.getWorld());
				screendoorEntity.refreshPositionAndAngles(blockPos, 0.0F, 0.0F);
				screendoorEntity.initialize(serverWorld, FlagPeasantEntity.this.getWorld().getLocalDifficulty(blockPos), SpawnReason.MOB_SUMMONED, (EntityData) null, (NbtCompound) null);
            ...
            for(int p = 0; p < 3; ++p) { // 3 Conehead
				...
				PeasantEntity coneheadEntity = (PeasantEntity) cone.create(FlagPeasantEntity.this.getWorld());
                coneheadEntity.refreshPositionAndAngles(blockPos, 0.0F, 0.0F);
                coneheadEntity.initialize(serverWorld, FlagPeasantEntity.this.getWorld().getLocalDifficulty(blockPos), SpawnReason.MOB_SUMMONED, (EntityData)null, (NbtCompound)null);
            ...
            for(int d = 0; d < 2; ++d) { // 2 Buckethead
				...
				PeasantEntity bucketheadEntity = (PeasantEntity) bucket.create(FlagPeasantEntity.this.getWorld());
                bucketheadEntity.refreshPositionAndAngles(blockPos, 0.0F, 0.0F);
                bucketheadEntity.initialize(serverWorld, FlagPeasantEntity.this.getWorld().getLocalDifficulty(blockPos), SpawnReason.MOB_SUMMONED, (EntityData)null, (NbtCompound)null);
            ...
            for(int t = 0; t < 6; ++t) { // 6 Browncoat
				...
				PeasantEntity browncoatEntity = (PeasantEntity) coat.create(FlagPeasantEntity.this.getWorld());
                browncoatEntity.refreshPositionAndAngles(blockPos, 0.0F, 0.0F);
                browncoatEntity.initialize(serverWorld, FlagPeasantEntity.this.getWorld().getLocalDifficulty(blockPos), SpawnReason.MOB_SUMMONED, (EntityData)null, (NbtCompound)null);
