...
public class BassZombieEntity extends ZombieRidersEntity implements GeoAnimatable {
	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
		if (this.hasVehicle()){
			if (this.getVehicle() instanceof SpeakerVehicleEntity) {
				event.getController().setAnimation(RawAnimation.begin().thenLoop("bass.idle"));
			}
			...
			event.getController().setAnimation(RawAnimation.begin().thenLoop("bass.ducky"));
			if (this.isIced) {
				event.getController().setAnimationSpeed(0.5);
			}
			else {
			...
			if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
				event.getController().setAnimation(RawAnimation.begin().thenLoop("bass.walking"));
			} else {
				event.getController().setAnimation(RawAnimation.begin().thenLoop("bass.idle2"));
			}
			if (this.isFrozen || this.isStunned) {
				event.getController().setAnimationSpeed(0);
			}
	...
	public boolean damage(DamageSource source, float amount) {
        if (!super.damage(source, amount)) {
            ...
            if (this.getRecentDamageSource() == PvZCubed.HYPNO_DAMAGE && !(this.getHypno())) {
                ...
                BassZombieEntity hypnotizedZombie = (BassZombieEntity) hypnoType.create(getWorld());
