	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
		if (this.hasVehicle()){
			if (this.getVehicle() instanceof SpeakerVehicleEntity) {
				event.getController().setAnimation(new RawAnimation().loop("bass.idle"));
			}
			...
			event.getController().setAnimation(new RawAnimation().loop("bass.ducky"));
			if (this.isIced) {
				event.getController().setAnimationSpeed(0.5);
			}
			else {
			...
			if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
				event.getController().setAnimation(new RawAnimation().loop("bass.walking"));
			} else {
				event.getController().setAnimation(new RawAnimation().loop("bass.idle2"));
			}
			if (this.isFrozen || this.isStunned) {
				event.getController().setAnimationSpeed(0);
			}
