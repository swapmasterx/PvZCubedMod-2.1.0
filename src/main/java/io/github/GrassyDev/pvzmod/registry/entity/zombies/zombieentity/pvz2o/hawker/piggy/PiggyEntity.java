...
public class PiggyEntity extends SmallAnimalEntity implements GeoEntity {
	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
		if (this.isInsideWaterOrBubbleColumn()) {
			event.getController().setAnimation(RawAnimation.begin().thenLoop("piggy.walk"));
			if (this.isIced) {
			...
			if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
				...
				event.getController().setAnimation(RawAnimation.begin().thenLoop("piggy.idle"));
			}
		}
	...
	public static DefaultAttributeContainer.Builder createZombiePiggyAttributes() {
        return HostileEntity.createAttributes().add(EntityAttributes.GENERIC_FOLLOW_RANGE, 75.0D)
				.add(ReachEntityAttributes.ATTACK_RANGE, 1.0D)
				.add(ReachEntityAttributes.REACH, 1.0D)
	...
	public boolean damage(DamageSource source, float amount) {
		if (!super.damage(source, amount)) {
			...
			if (this.getRecentDamageSource().isType(PvZDamageTypes.HYPNO_DAMAGE) && !(this.getHypno())) {
				...
				PiggyEntity hypnotizedZombie = (PiggyEntity) hypnoType.create(getWorld());
