...
public class TulimpeterSeeds extends SeedItem implements FabricItem {
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				TulimpeterEntity plantEntity = (TulimpeterEntity) PvZEntity.TULIMPETER.spawnFromItemStack((ServerWorld)world, itemStack, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			TulimpeterEntity plantEntity = PvZEntity.TULIMPETER.spawnFromItemStack((ServerWorld)world, stack, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.TULIMPETER.getDimensions().getBoxAt(plantEntity.getPos()));
			if (list.isEmpty()) {
			...
			TulimpeterEntity plantEntity = (TulimpeterEntity) PvZEntity.TULIMPETER.spawnFromItemStack((ServerWorld)world, stack, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			if (plantEntity == null) {
				return ActionResult.FAIL;
			}
