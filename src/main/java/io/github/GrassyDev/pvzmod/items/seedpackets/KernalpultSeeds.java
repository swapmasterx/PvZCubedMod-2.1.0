...
import io.github.GrassyDev.pvzmod.registry.entity.plants.plantentity.pvz1.roof.kernalpult.KernalpultEntity;
...
public class KernalpultSeeds extends SeedItem implements FabricItem {
    public boolean used;
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.kernalpultS() * 20);

    public KernalpultSeeds(Item.Settings settings) {
        super(settings);
    }
...
//	}
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.kernalpult_seed_packet.flavour")
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (direction == Direction.DOWN) {
			...
			Box box = PvZEntity.KERNALPULT.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				List<Entity> list = world.getNonSpectatingEntities(Entity.class, box);
				if (list.isEmpty()) {
					KernalpultEntity plantEntity = (KernalpultEntity) PvZEntity.KERNALPULT.spawnFromItemStack((ServerWorld)world, itemStack, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			if (list.isEmpty()) {
				plantEntity = PvZEntity.KERNALPULT.spawnFromItemStack((ServerWorld)world, stack, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
