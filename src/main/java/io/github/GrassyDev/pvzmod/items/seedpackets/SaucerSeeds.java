...
public class SaucerSeeds extends SeedItem implements FabricItem {
    ...
    public SaucerSeeds(Item.Settings settings) {
        super(settings);
    }
...
//	}
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		Box box = PvZEntity.SAUCER.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world instanceof ServerWorld serverWorld) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity) {
			if (list.isEmpty()) {
				plantEntity = PvZEntity.SAUCER.spawnFromItemStack((ServerWorld)world, stack, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
