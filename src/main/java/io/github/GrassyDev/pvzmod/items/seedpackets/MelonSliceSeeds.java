...
public class MelonSliceSeeds extends SeedItem implements FabricItem {
	...
	public MelonSliceSeeds(Item.Settings settings) {
		super(settings);
	}
...
//	}
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			Box box = PvZEntity.MELONSLICEPULT.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				List<Entity> list = world.getNonSpectatingEntities(Entity.class, box);
				if (list.isEmpty()) {
					MelonsliceEntity plantEntity = (MelonsliceEntity) PvZEntity.MELONSLICEPULT.spawnFromItemStack((ServerWorld)world, itemStack, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			if (list.isEmpty()) {
				plantEntity = PvZEntity.MELONSLICEPULT.spawnFromItemStack((ServerWorld)world, stack, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
