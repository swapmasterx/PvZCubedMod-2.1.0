package io.github.GrassyDev.pvzmod.items.rockobstacle;
...
public class BasicTombStone extends SeedItem {

	public BasicTombStone(Item.Settings settings) {
		super(settings);
	}
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		Box box = PvZEntity.BASICTOMBSTONE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
			RockObstacleEntity basicGraveEntity = (RockObstacleEntity) PvZEntity.BASICTOMBSTONE.spawnFromItemStack((ServerWorld)world, itemStack, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
